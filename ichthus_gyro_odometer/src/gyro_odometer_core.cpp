  ...
  imu_vel_msg_ptr_(nullptr)
{
  ...
  imu_vel_sub_ = create_subscription<geometry_msgs::msg::TwistStamped>(
    "/imu/velocity", rclcpp::QoS{100}, std::bind(&GyroOdometer::callbackImuVelocity, this, std::placeholders::_1));
  ...
  imu_twist_with_covariance_pub_ = create_publisher<geometry_msgs::msg::TwistWithCovarianceStamped>(
    "/imu/twist_with_covariance", rclcpp::QoS{10});
...
void GyroOdometer::callbackImuVelocity(const geometry_msgs::msg::TwistStamped::ConstSharedPtr imu_vel_msg_ptr)
{
  imu_vel_msg_ptr_ = imu_vel_msg_ptr;
}
...
void GyroOdometer::callbackImu(const sensor_msgs::msg::Imu::ConstSharedPtr imu_msg_ptr)
{
  ...
  geometry_msgs::msg::TwistWithCovarianceStamped imu_twist_with_covariance;
  imu_twist_with_covariance.header.stamp = imu_msg_ptr_->header.stamp;
  imu_twist_with_covariance.header.frame_id = output_frame_;
  imu_twist_with_covariance.twist.twist.linear.x = std::sqrt( 
    std::pow(imu_vel_msg_ptr_->twist.linear.x, 2) + 
    std::pow(imu_vel_msg_ptr_->twist.linear.y, 2) + 
    std::pow(imu_vel_msg_ptr_->twist.linear.z, 2));
  imu_twist_with_covariance.twist.twist.linear.y = 0.0; 
  imu_twist_with_covariance.twist.twist.linear.z = 0.0; 
  imu_twist_with_covariance.twist.twist.angular.x = transformed_angular_velocity.vector.x;
  imu_twist_with_covariance.twist.twist.angular.y = transformed_angular_velocity.vector.y;
  imu_twist_with_covariance.twist.twist.angular.z = transformed_angular_velocity.vector.z;

  imu_twist_with_covariance.twist.covariance[0] = twist_with_cov_msg_ptr_->twist.covariance[0];
  imu_twist_with_covariance.twist.covariance[7] = 10000.0;
  imu_twist_with_covariance.twist.covariance[14] = 10000.0;
  imu_twist_with_covariance.twist.covariance[21] = imu_msg_ptr_->angular_velocity_covariance[0];
  imu_twist_with_covariance.twist.covariance[28] = imu_msg_ptr_->angular_velocity_covariance[4];
  imu_twist_with_covariance.twist.covariance[35] = imu_msg_ptr_->angular_velocity_covariance[8];
  
  if (
    std::fabs(transformed_angular_velocity.vector.z) < 0.01 &&
    std::fabs(imu_twist_with_covariance.twist.twist.linear.x) < 0.01) {
    imu_twist_with_covariance.twist.twist.linear.x = 0.0;
    imu_twist_with_covariance.twist.twist.linear.y = 0.0;
    imu_twist_with_covariance.twist.twist.linear.z = 0.0;
    imu_twist_with_covariance.twist.twist.angular.x = 0.0;
    imu_twist_with_covariance.twist.twist.angular.y = 0.0;
    imu_twist_with_covariance.twist.twist.angular.z = 0.0;
  }
  imu_twist_with_covariance_pub_->publish(imu_twist_with_covariance);
