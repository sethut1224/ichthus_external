...
: Node{"recordreplay_planner", node_options}
{
  ...
  m_recording_frame = declare_parameter("recording_frame", "map");
  ...
  // Set up services
  if (declare_parameter("enable_object_collision_estimator", false)) {
...
void RecordReplayPlannerNode::on_ego(const nav_msgs::msg::Odometry::SharedPtr & msg)
{
  if (m_odom_frame_id.empty()) {
    m_odom_frame_id = msg->header.frame_id;
  }
  ...
  state_msg.state.longitudinal_velocity_mps = msg->twist.twist.linear.x;
  state_msg.state.lateral_velocity_mps = msg->twist.twist.linear.y;
  state_msg.state.heading_rate_rps = msg->twist.twist.angular.z;

  Transform tf_map2odom;
  State msg_world = state_msg;
  try {
    tf_map2odom = tf_buffer_->lookupTransform(
      m_recording_frame, msg->header.frame_id,
      tf2::TimePointZero);
  ...
  motion::motion_common::doTransform(state_msg, msg_world, tf_map2odom);
  msg_world.header.frame_id = m_recording_frame;
