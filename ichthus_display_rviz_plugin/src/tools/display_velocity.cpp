...
VelocityDisplay::VelocityDisplay()
{
  ...
  const auto left = static_cast<int>(std::round(1650 * scale));
  const auto top = static_cast<int>(std::round(64 * scale));
  const auto length = static_cast<int>(std::round(256 * scale));

  property_text_color_ = new rviz_common::properties::ColorProperty(
    "Text Color", QColor(255, 0, 0), "text color", this, SLOT(updateVisualization()), this);
...
void VelocityDisplay::onInitialize()
{
  RTDClass::onInitialize();
  static int count = 0;
  rviz_common::UniformStringStream ss;
  ss << "VelocityDisplayObject" << count++;
...
void VelocityDisplay::onEnable()
{
  subscribe();
  overlay_->show();
}

void VelocityDisplay::onDisable()
{
  unsubscribe();
  reset();
  overlay_->hide();
...
void VelocityDisplay::update(float wall_dt, float ros_dt)
{
  (void)wall_dt;
  (void)ros_dt;
  ...
  hud.fill(background_color);overlay_->updateTextureSize(property_length_->getInt(), property_length_->getInt());
  overlay_->setPosition(property_left_->getInt(), property_top_->getInt());
  overlay_->setDimensions(overlay_->getTextureWidth(), overlay_->getTextureHeight());
  ...
  velocity_ss << std::fixed << std::setprecision(2) << linear_x << "km/h";
  painter.drawText(
...
void VelocityDisplay::processMessage(const std_msgs::msg::Float64::ConstSharedPtr msg_ptr)
{
  // (void)msg_ptr;
  if (!isEnabled()) {
    return;
...
void VelocityDisplay::updateVisualization()
{
  // overlay_->updateTextureSize(property_length_->getInt(), property_length_->getInt());
  // overlay_->setPosition(property_left_->getInt(), property_top_->getInt());
  // overlay_->setDimensions(overlay_->getTextureWidth(), overlay_->getTextureHeight());
...
PLUGINLIB_EXPORT_CLASS(rviz_plugins::VelocityDisplay, rviz_common::Display)
